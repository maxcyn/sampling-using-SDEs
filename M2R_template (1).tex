...
cost-aware efficiency metric based on gradient evaluations, and studies
...
because the gradient is the only expensive operation in realistic
models, the number of gradient evaluations is the natural unit of
computational cost; we return to this point when comparing samplers in
...
The price of the correction is one additional gradient evaluation per
iteration: the proposed point's gradient is required both for the
reverse proposal density in \eqref{eq:impl-accept} and as the cached
gradient should the move be accepted. MALA therefore costs roughly twice
as much per step as ULA, a difference we account for explicitly when we
measure efficiency per gradient evaluation later. In exchange, MALA has
...
sampler with ULA and MALA using accuracy, ESS, and ESS per gradient
evaluation.
...
MALA is charged two gradient evaluations per iteration because the
Metropolis ratio requires the gradient at the proposed point. This
...
%   ESS in the title. Export that figure to fig/gaussian_trace_acf.pdf
%   and uncomment the \includegraphics line below.
%----------------------------------------------------------------------
\begin{figure}[ht]
\centering
\includegraphics[width=0.9\textwidth]{fig/gaussian_trace_acf.png}
...
% Three panels: covariance error, mean error, minimum ESS, vs step size.
%----------------------------------------------------------------------
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{fig/gaussian_ula_mala_sweep.png}
\caption{ULA versus MALA on the correlated Gaussian, as a function of
step size $h$. \emph{Left:} covariance error in Frobenius norm.
\emph{Centre:} mean error. \emph{Right:} minimum-coordinate effective
sample size (higher is better). ULA's covariance error grows with $h$,
...
freely but is accurate only at small step sizes; MALA pays for an extra
gradient and the occasional rejection but removes the bias entirely. The next two experiments carry this comparison into Bayesian linear regression, where the posterior is again Gaussian and the exact moments remain available, and then into logistic regression, where the posterior is non-Gaussian and a long reference chain must stand in for the unavailable truth. The same diagnostics are later reused for the kinetic sampler of Section~\ref{sec:extension}.
...
most stable on covariance and functional accuracy over the useful part of
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{fig/linear_regression_cost_aware.png}
\caption{Cost-aware Bayesian linear regression comparison. The three
panels show functional RMSE, covariance error, and ESS per gradient
evaluation against step size.}
...
ESS per gradient evaluation as the step size grows, but this is matched
by worsening functional and covariance error. MALA is more stable on the
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{fig/logistic_regression_cost_aware.png}
\caption{Cost-aware Bayesian logistic regression comparison against the
long MALA reference chain. The three panels show functional RMSE,
covariance error, and ESS per gradient evaluation against step size.}
...
using the same post-burn-in diagnostics as before, together with a
functional RMSE over posterior summaries and ESS per gradient evaluation.
...
ESS per gradient evaluation because it never rejects and uses only one
gradient per step, but its covariance error remains visibly affected by
finite-step bias. MALA is the most accurate method in this grid, with much
lower functional RMSE and covariance error, but this comes at the cost of
extra gradient evaluations and rejections. KLMC sits between these two
...
rate, and gradient cost--remain informative when exact posterior moments
...
while its functional error worsens, MALA remains stable on covariance
error, and KLMC can be competitive for a well-tuned step size without a
...
There are several caveats to the numerical comparisons. First, the ESS reported here is the minimum coordinate-wise ESS, not a multivariate ESS, and values near the post-burn-in chain length should be read as diagnostic summaries rather than exact independent sample counts. Second, the cost normalisation uses gradient evaluations as the main unit because gradients dominate many Bayesian models, but this ignores log-density evaluations, proposal-density arithmetic, linear algebra, and vectorisation effects. This convention is especially important for MALA, whose implementation can cache one current gradient but still requires extra target/proposal work for the accept--reject step. Third, the logistic-regression errors are distances to a long MALA reference chain rather than exact posterior errors; no multi-chain convergence or Monte Carlo standard error analysis is included. Finally, KLMC is unadjusted in these experiments, so high ESS can still correspond to efficient exploration of a biased discretisation.
