...
section only states how those quantities are used in the result tables
and figures. For a post-burn-in chain with sample mean $\hat{\mu}$ and
sample covariance $\hat{\Sigma}$, the two accuracy diagnostics are
\[
e_\mu = \|\hat{\mu}-\mu_{\mathrm{ref}}\|_2,
\qquad
e_\Sigma = \|\hat{\Sigma}-\Sigma_{\mathrm{ref}}\|_F .
\]
The first measures error in the posterior centre, while the second
measures error in the posterior spread and correlation structure. For
the Gaussian and linear-regression experiments, $(\mu_{\mathrm{ref}},
\Sigma_{\mathrm{ref}})$ are exact analytic posterior moments; for
logistic regression, they are estimated from the long MALA reference
chain. Mixing is summarised by
\[
\mathrm{ESS}_{\min}=\min_{1\le j\le d}\mathrm{ESS}_j,
\]
the worst coordinate-wise ESS from Section~\ref{sec:impl-diagnostics}.
Finally, computational cost is reported through
\[
W = N_{\nabla} + N_{\log\pi},
\qquad
\mathrm{ESS}_{\mathrm{work}}=\frac{\mathrm{ESS}_{\min}}{W},
\]
